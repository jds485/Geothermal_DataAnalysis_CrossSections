...
MakeGeothermPlot = function(Wells,              #The well database
                            ...
                            ColRampCols         #Vector of colors to use in the color ramp. e.g. c("red", "orange", "yellow", "green", "blue")
                            ){
  ...
  if (is.na(ColRampDivs)==FALSE){
    ...
    color = rep("blue", length(Wells[,1]))
  }
  
  for (i in 1:length(Wells[,1])){
    if (i == 1){
      plot(y = Depths, x = as.numeric(Wells[i,(which(colnames(Wells)==StartTempField):length(Wells[1,]))]), ylim=c(MaxDepth,MinDepth), xlim=c(MinTemp, MaxTemp), type='l', lwd=1.5, col=color[i], xlab=expression(paste("Temperature (",degree,"C)")), ylab="Depth (m)", main="Geotherms for wells in Tompkins County, NY", cex.lab=1.5, cex.main=1.5, cex.axis=1.5)
      par(new=TRUE)
      if (is.na(BasementDepth) == FALSE){
        plot(y = Wells[BasementDepth][i,1], x = as.numeric(Wells[i,(which(colnames(Wells)==StartTempField)-1+round(Wells[BasementDepth][i,1]/10))]), ylim=c(MaxDepth,MinDepth), xlim=c(MinTemp, MaxTemp), pch='-', cex=1.5, xlab='', ylab='', axes=FALSE)
        par(new=TRUE)
      }
    }
    else{
...
#Example plot:
png("ExampleGeothermPlotTompkinsNY.png", width=8, height=8, units="in", res=150)
MakeGeothermPlot(Wells = Wells, BHT = 'BHT', WellDepth = 'WellDepth', StartTempField = 'T0', BasementDepth = 'BasementDepth', MinDepth = 10, MaxDepth = 5000, MinTemp = 0, MaxTemp = 150, DepthIncrement = 10, Trend = 'LatDegr', ColRampDivs=length(Wells[,1]), ColRampCols = c("green", "yellow", "orange", "red"))
legend(x = 0, y=3000, legend=c("BHTs", "Basement Depth","Geotherms", "Southern Tomkins County", "Northern Tomkins County"), pch=c(16,95,NA,NA,NA), lty=c(NA,NA,1,1,1), col=c("black","black","black","green","red"), bty="n")
dev.off()
